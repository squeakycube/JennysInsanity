...
using JennysInsanity.Projectiles;
...
namespace JennysInsanity.Items
{
	...
	public class SamanualsSpecialItem : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			item.rare = ItemRarityID.Blue; // Sets the item's rarity.
			item.UseSound = SoundID.Item34;
			item.autoReuse = true;
			//item.shoot = ModContent.ProjectileType<FlamethrowerProjectile>();
            item.shoot = mod.ProjectileType("FlamethrowerProjectile");
		...
		public override void AddRecipes()
		{
			...
			//Molten Trident here whenever I have the id
			//recipe.AddIngredient(ModContent.ItemType<ExampleItem>(), 3);
			//Lothar gel can only be bought from lothar or dropped from goblins, research editing vanilla drops
			//recipe.AddIngredient(ModContent.ItemType<*ADD LOTHAR GEL TO THIS*>(), 3);
			recipe.AddIngredient(ItemID.CursedFlame, 15); //Shadowflame Hex Doll
			//recipe.AddIngredient(ItemID.ShadowflameHexDoll, 3); 
